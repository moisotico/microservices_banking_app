...
func (d AccountRepositoryDb) SaveTransaction(t Transaction) (*Transaction, *errs.AppError) {
	...
	//Rollback and changes from both the tables will reverted in case of error
	if err != nil {
		tx.Rollback()
	...
	if err != nil {
		tx.Rollback()
		logger.Error("Error while commiting transaction for the bank account: " + err.Error())
		return nil, errs.NewUnexpectedError("Unexpected database error")
	}
	...
	// update the transaction struc with the latest balance
...
func (d AccountRepositoryDb) FindBy(accountId string) (*Account, *errs.AppError) {
	...
	if err != nil {
		logger.Error("Error while fetching account info: " + err.Error())
		return nil, errs.NewUnexpectedError("Unexpected database error")
	}
	return &account, nil
}
